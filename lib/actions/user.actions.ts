...
export const signUp = async ({ password, ...userData }: SignUpParams) => {
    ...
    try {
        const { account, database } = await createAdminClient(); // create a new Appwrite client with an admin key
        ...
        return parseStringify(newUser); // return the new user
...
export const createLinkToken = async (user: User) => { // create a new link token in plaid
    try {
        const tokenParams = {
            user: {
...
export const createBankAccount = async ({
    ...
    try {        // create a new bank account
        ...
        return parseStringify(bankAccount); // return the bank account
    } catch (error) {
        console.error('Error', error);
    }
}
